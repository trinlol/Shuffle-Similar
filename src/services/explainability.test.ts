...
describe("Similar Mix explanations", () => {
  it("states real source provenance and a measurable music match", () => {
    ...
    expect(explainSimilarMixTrack(candidate, {
      ...
      genres: [],
      tempo: 120,
    })).toBe("Selected from track radio and your listening library; it has a close tempo to the seed track.")
  })

  it("does not invent a source when metadata does not contain one", () => {
    expect(explainSimilarMixTrack({ uri: "spotify:track:next" }, null))
      .toBe("Selected to balance similarity, discovery, and variety.")
  })
})
